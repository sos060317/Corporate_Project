...
namespace AssetKits.ParticleImage
{
    ...
    public sealed class ParticleImage : MaskableGraphic
    {
        ...
        private NativeArray<SpriteSheet> _sheetsArray;
        
        public NativeArray<SpriteSheet> sheetsArray
        {
            get
            {
                if(_sheetsArray.IsCreated == false)
                    UpdateSheets();
        ...
        public ParticlePool pool
        {
            get
            {
                if (_pool == null)
                {
                    _pool = new ParticlePool(128,this);
                }
        ...
        protected override void OnDisable()
        {
            ...
            if(_sheetsArray.IsCreated)
                _sheetsArray.Dispose();
        }
        ...
        { 
            if(_sheetsArray.IsCreated)
                _sheetsArray.Dispose();
            _sheetsArray = new NativeArray<SpriteSheet>(textureTile.x * textureTile.y, Allocator.Persistent);
            ...
            if (textureSheetEnabled)
            {
                for (int i = textureTile.y-1; i > -1; i--)
                {
                    for (int j = 0; j < textureTile.x; j++)
                    {
                        ...
                        _sheetsArray[i * textureTile.x + j] = new SpriteSheet(new Vector2(1f/textureTile.x * (j+1),1f/textureTile.y * (i+1)), new Vector2(1f/textureTile.x * j,1f/textureTile.y * i));
                    }
                }
            }
            else
