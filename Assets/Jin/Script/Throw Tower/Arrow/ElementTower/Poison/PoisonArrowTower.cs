...
public class PoisonArrowTower : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (poisonArrowTowerUI.buttonDown == true)
        {
            isClicked = false;
            RoundObject.SetActive(false);
            poisonArrowTowerUI.buttonDown = false;
        }
    ...
    private void SpawnArrowsAtPositions()
    {
        foreach (GameObject spawnPosition in spawnPositions)
        {
            ...
            {
                var temp = Instantiate(arrowPrefab, spawnPosition.transform.position, Quaternion.identity).GetComponent<PoisonArrow>();

                temp.poisonArrowTower = GetComponent<PoisonArrowTower>();
            }
        }
    }
